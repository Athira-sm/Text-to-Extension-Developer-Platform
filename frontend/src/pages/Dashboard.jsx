...
function Dashboard() {
  ...
  const editExtension = async (id, oldPrompt) => {
    ...
    await axios.put(`${API_BASE_URL}/api/extension/${id}`, {
      newPrompt,
    });
  ...
  const deleteExtension = async (id) => {
    ...
    await axios.delete(`${API_BASE_URL}/api/extension/${id}`);
    fetchData();
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-[#0f2027] via-[#203a43] to-[#2c5364] text-white p-6">
      ...
      <div className="grid gap-4">
        {extensions.map((ext) => (
          <div
            key={ext._id}
            className="bg-white/10 backdrop-blur-lg border border-white/20 p-5 rounded-xl shadow-md"
          >
